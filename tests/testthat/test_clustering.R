describe("clustering()", {
    ...
    it("runs without errors on basic settings", {
        r <- ctest_for_no_errors(error_message = "clustering() does not run on defaults",
                                 to_eval = clustering(train = dataset1))
        r <- ctest_for_no_errors(error_message = "clustering() does not run on defaults with exclusion",
    ...
    it("can run when not excluding missing values or non-numeric columns", {
        r <- clustering(train = dataset1, exclude_columns = character(0))
    })
    ...
    it("adds cluster labels to a dataset as a character vector", {
        ...
        r <- clustering(train = test_df, cluster_column = clust_column, k = 2)
    ...
    it("allows you to set the number of clusters", {
        k <- 9
        r <- clustering(train = dataset1, k = k)
    ...
    it("allows you to set the distance metric", {
        r <- ctest_for_no_errors(error_message = "clustering() does not run on defaults",
                            to_eval = clustering(train = dataset1, metric = "manhattan"))
    })
})
