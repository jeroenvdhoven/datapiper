...
pipe_one_hot_encode <- function(train, columns = colnames(train)[purrr::map_lgl(train, function(x) return(!(is.numeric(x) || is.logical(x))))],
    ...
    if(missing(stat_functions)){
        ...
        generated_columns <- colnames(stats_transformer$train)[!colnames(stats_transformer$train) %in% original_columns]
        if(is.data.table(train_dt_set)) {
            train_dt_set <- train_dt_set[, .SD, .SDcols = generated_columns]
            train[, c(generated_columns) := NULL]
...
feature_one_hot_encode_predict <- function(data, one_hot_parameters, use_pca, pca, stat_transformer, generated_columns){
    ...
    if(!is.pipe(stat_transformer) && !is.pipeline(stat_transformer)){
        ...
        if(is.data.table(test_dt_set)) {
            test_dt_set <- test_dt_set[, .SD, .SDcols = generated_columns]
            data[, c(generated_columns) := NULL]
...
pipe_categorical_filter <- function(
    ...
    for(i in seq_along(categorical_columns)) {
        ...
        if(is.data.table(train)) {
            valid_categories <- unlist(train[, .N, by = col][N >= threshold][, col, with = F])
        } else {
            tabulated <- group_by_(train, col) %>% summarize(n = n()) %>% ungroup %>%
                filter(threshold <= n)
            valid_categories <- unlist(tabulated[, col])
        }

        mappings[[i]] <- valid_categories
    }
...
feature_categorical_filter_predict <- function(data, categorical_columns, mappings, insufficient_occurance_marker) {
    ...
    filter_vector <- function(vec, possible_values, insufficient_occurance_marker) {
        ifelse(vec %in% possible_values, vec, insufficient_occurance_marker)
    }
    ...
    for(i in seq_along(categorical_columns)) {
        ...
        if(is.data.table(data)) {
            data[, c(col) := filter_vector(get(col), mapping, insufficient_occurance_marker)]
        } else {
            data[col] <- filter_vector(unlist(data[col]), mapping, insufficient_occurance_marker)
        }
    }
